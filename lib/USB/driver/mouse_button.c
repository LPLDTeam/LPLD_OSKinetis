 ...
 *****************************************************************************/
uint_8 kbi_stat;	   /* Status of the Key Pressed */
...
void USB_HID_MouseControl(uint8 * cmd_buf)
{
  ...
  if(cmd_buf[0] > 0 || cmd_buf[1] > 0 || cmd_buf[2] > 0)
  {
    memcpy(rpt_buf,cmd_buf,sizeof(rpt_buf));
    memset(cmd_buf,0x00,sizeof(rpt_buf));
